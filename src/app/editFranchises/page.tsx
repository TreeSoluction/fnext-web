...
import { GetFranchiseByOwner } from "@/services/Franchises/GetFranchiseByOwner.owner";
...
export default function Franchise() {
  ...
  useEffect(() => {
    const loadFranchise = async () => {
      const result = await GetFranchiseByOwner(owner.id);
      setFranchise(result);
    };
  ...
  useEffect(() => {
    if (franchise != null) {
      setNameState(franchise?.name);
      setLogoImg(franchise?.logo);
      setHandleSite(franchise?.site);

      setHeadquarters(franchise.units_in_brazil);
      setMonthlyRevenue(franchise?.average_monthly_billing);
      setReturnonInvestmenFrom(franchise?.ROI_min);
      setReturnonInvestmenUntil(franchise?.ROI_max);
    }
  }, [franchise]);
