...
export default function FilterSegments() {
  ...
  const handleScroll = (scrollOffset: number) => {
    if (segmentsRef.current) segmentsRef.current.scrollLeft += scrollOffset;
  };
  ...
  return (
    <div className="flex items-center justify-center gap-8">
      ...
      <ul className="flex flex-grow overflow-x-hidden" ref={segmentsRef}>
        {values.map((value) => (
          <li
            className="flex flex-col flex-shrink-0 items-center justify-center mx-4 relative"
            key={value}
          >
            <img
              height={40}
              width={40}
              src={ICONS.segments.turism_agency}
              alt="Agências de Viagens e Turism"
            />
            <p className="text-xs text-center font-bold">Texto Escrito</p>
            <input
              type="radio"
              name="segment"
              className="absolute w-full h-full peer opacity-0"
            />
      ...
      <div className="flex-shrink-0">
        <button>
          <img
            ...
            alt="Filtro"
          />
          <p className="text-xs text-center text-MAIN_HIGH_BLUE font-bold -indent-1">
            Filtrar
          </p>
        </button>
      </div>
