 ...
 */
export default function generateApiUrl(endpoint: Endpoint, { params, query }: ConfigParams): string {

    let final_endpoint = API_ENDPOINTS[endpoint];

    if (params) {
        Object.keys(params).forEach((key) => {
            final_endpoint = final_endpoint.replace(`:${key}`, params[key].toString());
        });
    }
